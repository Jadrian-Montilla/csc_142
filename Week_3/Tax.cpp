...
/*
    Tax
    The program here multiples the sales tax of the state and applies it to the item you are purchasing.
    Jadrian Montilla
    2/3/25
*/
...
int main() {
    ...
    do{
        std::cout << "Describe the item you are purchasing: ";
        std::getline(std::cin, desc);
        std::cout << "What is the price of your item: ";
        std::getline(std::cin, cost);
        numcost = std::stod(cost);
        if (numcost > 0){
            std::cout << "\nItem name: " << desc << std::endl;
            std::cout << "Regular cost of " << desc << ": $" << std::fixed << std::setprecision(2) << numcost << std::endl;

            salescost = numcost * TAX;
            std::cout << "Sales tax: $" << std::fixed << std::setprecision(2) << salescost << std::endl;

            taxedcost = salescost + numcost;
            std::cout << "\nCost after tax: $" << std::fixed << std::setprecision(2) << taxedcost << std::endl;
        }
        else
        {
            std::cout << "Oh dear! The cost is not supposed to be a negative.\n" << std::endl;
        }

        std::cout << "Would you like to re-enter? (y/n)\n";
