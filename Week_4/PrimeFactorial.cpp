...
/*
    Prime Factorial
    This program randomly generates a number between 0 and 50, 
    analyzes whether the chosen number is prime or not (excluding 0 and 1),
    and presents its factorial (if it's a number the program can't comprehend it 
    replaces the number with "a really big number" due to it automatically becoming a 0).
    Jadrian Montilla
    2/13/25
*/
...
int main(){
    ...
    do
    {
        ...
        {
            std::cout << number << " is the chosen prime number." << std::endl;
            if (factor == 0)
            {
                std::cout << "Its factorial is a really big number." <<  std::endl;
            }
            else
            {
                std::cout << "Its factorial is " << factor << "." << std::endl;
            }
